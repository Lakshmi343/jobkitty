    ...
    const validateStep = (step) => {
        ...
        switch (step) {
            ...
            case 3:
                ...
                if (input.salaryMax === undefined || input.salaryMax === "") {
                    setInput(prev => ({ ...prev, salaryMax: "0" }));
                }
                
                if (input.openings === undefined || input.openings === "") {
                    setInput(prev => ({ ...prev, openings: "1" }));
                }
                
                // If values are provided, convert to numbers and validate
                const minSalary = parseFloat(input.salaryMin || 0);
                const maxSalary = parseFloat(input.salaryMax || 0);
                const openingsCount = parseInt(input.openings || 1);
                
                // Swap salary values if min > max
                if (minSalary > maxSalary) {
                    setInput(prev => ({
                        ...prev,
                        salaryMin: String(maxSalary),
                        salaryMax: String(minSalary)
                    }));
                }
                
                return true;
            case 4:
                // Experience level and requirements are optional
                
                // Set default values for empty fields
                if (!input.experienceLevel) {
                    setInput(prev => ({ ...prev, experienceLevel: "Entry Level" }));
                }
                
                if (input.experienceMin === undefined || input.experienceMin === "") {
                    setInput(prev => ({ ...prev, experienceMin: "0" }));
                }
                
                if (input.experienceMax === undefined || input.experienceMax === "") {
                    setInput(prev => ({ ...prev, experienceMax: "5" }));
                }
                
                // If values are provided, validate them
                const expMin = parseFloat(input.experienceMin || 0);
                const expMax = parseFloat(input.experienceMax || 5);
                
                // Swap experience values if min > max
                if (expMin > expMax) {
                    setInput(prev => ({
                        ...prev,
                        experienceMin: String(expMax),
                        experienceMax: String(expMin)
                    }));
                }
                
                // If no requirements, set a default
